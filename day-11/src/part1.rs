...
impl Map {
    fn distance(&self, a: &Point, b: &Point) -> i64 {
        ...
        a.distance(b) + crossed_rows + crossed_cols
    }
}
...
#...#.....";
    ...
    fn test_map_distance() -> Result<()> {
        let map: Map = TEST_INPUT.parse()?;
        assert_eq!(map.distance(&map.galaxies[4], &map.galaxies[8]), 9);
        assert_eq!(map.distance(&map.galaxies[0], &map.galaxies[6]), 15);
        assert_eq!(map.distance(&map.galaxies[2], &map.galaxies[5]), 17);
        assert_eq!(map.distance(&map.galaxies[7], &map.galaxies[8]), 5);
        Ok(())
    }
